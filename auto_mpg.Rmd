...
```{r Try using modelr but failed, include=FALSE}
# library(modelr)
# cv_df =
#   crossv_mc(df, 100, test = 0.3)
# 
# ctrl <- trainControl(method = "repeatedcv",
#                      summaryFunction = twoClassSummary,
#                      classProbs = TRUE)
...
#   cv_df %>%
#   mutate(
#     glm.fit = map(train, ~glm(mpg_cat ~ .,data = .x, family = binomial(link = "logit"))),
#     mars.fit = map(train, ~caret::train(mpg_cat ~ ., data = .x, method = "earth")))
...
```{r, include=FALSE}
# set.seed(7)
# tmp <- createDataPartition(y = df$mpg_cat,
#                            p = .7,
#                            times = 20)
# 
# glm.models = 
...
#                     method = "LGOCV", classProbs = TRUE, index = tmp))
# 
# mars.models <- 
#   train(mpg_cat ~ .,
#         data = df,
